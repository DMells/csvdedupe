...
def writeResults(clustered_dupes, input_file, output_file):
    ...
    for cluster_id, (cluster, score) in enumerate(clustered_dupes):
        for record_id in cluster:
            # DM added score here. Score is a multi-element list therefore only assign the score element which relates to that record/row_id.
            cluster_membership[record_id] = [cluster_id, score[cluster.index(record_id)]]
    ...
    heading_row.insert(0, u'Confidence Score')
    ### Manually added confidence score
    ...
    for row_id, row in enumerate(reader):
        if row_id in cluster_membership:
            ...
            # manually added score DM (see line 72)
            score = cluster_membership[row_id][1]
        else:
            cluster_id = unique_record_id
            unique_record_id += 1
            #CANCELLED -  DM Changed score from 0 to 1 for those that have their own cluster to themselves. Assuming 100% confidence rather than 0%.
            score = 0
        row.insert(0, cluster_id)
        # DM Manual addition of score to dataframe row here
        row.insert(1, score)
        writer.writerow(row)
